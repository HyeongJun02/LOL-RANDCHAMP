/* 계정당 저장 한도.
   ...
   진짜 방어선은 DB 트리거다(sql/setup.sql). Data API는 브라우저가 DB를
   직접 찌르는 구조라, 여기 숫자는 우회하면 그만이다.
   이 파일의 역할은 "한도를 넘기 전에 곱게 알려주기"까지다.
   숫자를 바꾸면 sql/setup.sql의 트리거도 같이 바꿔야 한다. */
