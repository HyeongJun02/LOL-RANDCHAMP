...
import { withinLimit, isAdmin, ADMIN_USER_ID, MAX_ROSTER, MAX_MATCHES } from './limits';
...
test('관리자는 한도가 없다', () => {
  ...
  expect(withinLimit('matches', 9999, ADMIN_USER_ID)).toBe(true);
});
...
test('일반 사용자는 한도까지만', () => {
  ...
  expect(withinLimit('matches', MAX_MATCHES, me)).toBe(true);
  expect(withinLimit('matches', MAX_MATCHES + 1, me)).toBe(false);
});

/* 화면과 DB의 숫자가 어긋나면, 화면에서는 저장되는데 서버가 거절하는
   상황이 된다. 한쪽만 고치는 실수를 여기서 잡는다. */
test('DB 트리거의 숫자가 앱과 같다', () => {
  expect(sql).toContain(`max_roster  constant int := ${MAX_ROSTER};`);
  expect(sql).toContain(`max_matches constant int := ${MAX_MATCHES};`);
});
