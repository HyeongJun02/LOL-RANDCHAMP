...
test('이긴 사람은 오르고 진 사람은 내린다 (합은 0)', () => {
  const s = statsFor(duel(1, 'A'), 'normal');
...
test('승패가 같으면 점수가 0 근처에 머문다', () => {
  ...
  expect(Math.abs(pointsOf(statsFor(list, 'normal'), '철수'))).toBeLessThanOrEqual(1);
});
...
test('이미 강한 상대를 이기면 약한 상대를 이길 때보다 많이 오른다', () => {
  ...
  const beatStrong = statsFor([...seed, game(['도전자'], ['고수'], 'A')], 'normal');
  clock = 0;
  const seed2 = Array.from({ length: 8 }, () => game(['고수'], ['호구'], 'A'));
  const beatWeak = statsFor([...seed2, game(['도전자'], ['호구'], 'A')], 'normal');
...
test('판수가 적으면 같은 전승이라도 점수가 낮다', () => {
  const few = pointsOf(statsFor(duel(2, 'A'), 'normal'), '철수');
  clock = 0;
  const many = pointsOf(statsFor(duel(20, 'A'), 'normal'), '철수');
  expect(many).toBeGreaterThan(few);
});

test('2판 2승이 폭주하지 않는다', () => {
  expect(pointsOf(statsFor(duel(2, 'A'), 'normal'), '철수')).toBeLessThan(K_FACTOR);
});

test('승패 수는 그대로 센다', () => {
  const s = statsFor([...duel(3, 'A'), game(['철수'], ['영희'], 'B')], 'normal');
  expect(s.get('철수')).toMatchObject({ wins: 3, losses: 1, games: 4 });
});

test('모드가 다르면 섞이지 않는다', () => {
  const list = [game(['철수'], ['영희'], 'A', 'aram')];
  expect(pointsOf(statsFor(list, 'normal'), '철수')).toBe(0);
  expect(pointsOf(statsFor(list, 'aram'), '철수')).toBeGreaterThan(0);
});

test('기록 순서가 뒤섞여 들어와도 시간순으로 계산한다', () => {
  const ordered = [...duel(3, 'A')];
  const shuffled = [ordered[2], ordered[0], ordered[1]];
  expect(pointsOf(statsFor(shuffled, 'normal'), '철수')).toBe(
    pointsOf(statsFor(ordered, 'normal'), '철수')
  );
});
